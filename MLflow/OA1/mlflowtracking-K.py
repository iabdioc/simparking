...
if __name__ == "__main__":
	...
	client = MlflowClient()
	experiment_name = "K sklearn simparking"
	exp = client.get_experiment_by_name(experiment_name)

	if not exp:
		mlflow.create_experiment(experiment_name,
			tags={'mlflow.note.content':'simparking variació de paràmetre K'})
		mlflow.set_experiment_tag("version", "1.0")
		mlflow.set_experiment_tag("scikit-learn", "K")
		exp = client.get_experiment_by_name(experiment_name)

	mlflow.set_experiment("K sklearn simparking")
	...
	for K in Ks:
		...
		clustering_model = clustering_kmeans(parking_data_gb_norm, K)
